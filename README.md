...
📖 **从零到验证的保姆级教程：[docs/tutorial.html](docs/tutorial.html)** —— **新手从这里开始**。下面四份是它的来源，按需查阅。
...
五份文档克隆后用浏览器直接打开即可。本仓库其余部分是**软件实现**。

## 当前状态

软件侧已全部就绪。这个项目真正的难点从来不是代码，而是**参数**——骨导补偿 EQ 的
曲线长什么样、混合比多少才"像"。这些只能靠实测回答。

| 项目 | 状态 |
| --- | --- |
| Python 参考实现（处理链、旋钮映射） | ✅ 已完成 |
| 分块与整段结果逐样本一致 | ✅ 已验证 |
| STM32 固件核心 | ✅ 与 Python 逐位一致（11 个用例） |
| Cortex-M7 交叉编译 | ✅ 通过，目标码约 4 KB |
| 实时调参界面 | ✅ 已完成 |
| **概念验证：音色像不像** | ⛔ 等硬件 |
| 佩戴位置、啸叫余量 | ⛔ 等硬件 |
...
按 [阶段 0 实验手册](docs/phase0-protocol.html) 执行：

1. **取得 V2S200D 引脚编号**（当前唯一阻塞项，`knowles.com/V2S` 或找经销商要 User Guide）
2. 打样两块传感器小板，配 NUCLEO-H743ZI2 验证 PDM 采集与声学耦合
3. 录制真实双通道信号，用调参界面找到"融"起来的参数
4. 参数收敛后进入第二阶段（腰包主板）

评估套件（KAS-33100-0006）成本较高，故改为自制传感器板；原理图见
[docs/sensor-boards.html](docs/sensor-boards.html)。
